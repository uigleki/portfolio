...
這是大學時期（2022-2023）的作品，代碼品質不佳（硬編碼選擇器、缺少類型標註），但**設計思路是對的**。保留這個專案是為了展示完整的學習軌跡。
...
這個腳本在東華大學實際使用時成功搶到了熱門課程。雖然代碼品質不佳，但核心邏輯（三階段時間控制 + 競爭處理）是正確的。
...
- [grabber.py](./grabber.py) - 選課自動化腳本（邏輯正確，代碼品質待改進）
