...
此倉庫**僅供求職審閱**，採用 [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) 授權：
...
- Zod schema 強制 AI 輸出結構化 JSON（避免 AI 亂回答）
- 特地用 TypeScript 而非 Python，展現多語言能力

---

### [NixOS Dotfiles](https://github.com/uigleki/dotfiles) - 聲明式環境管理 (2025/07 - 2025/10)
...
- 配置即代碼：用 Git 管理系統狀態，就像管理專案代碼
- 原子更新：系統更新失敗可即時回滾，不會搞壞環境
- 完全可重現：在任何機器上執行同一份配置，結果完全一致
...
⚠️ **早期專案**：代碼品質較低（硬編碼選擇器、缺少類型標註），但設計思路嚴謹。
...
⚠️ **早期專案**（已升級至 NixOS）：720 行 Bash 腳本，從磁碟分割到系統加固一鍵完成。
...
從基礎自動化 → Linux 系統管理 → 交易工具開發 → 數據分析 → 聲明式配置 → AI 工程的完整成長路徑。

---

## 關於我

資訊管理系畢業，專注於系統自動化和實用工具開發。我相信好的代碼不只是能跑，更要好維護、好擴展。熟悉 Python、C#、TypeScript、Nix、Bash 等多種語言，善於選擇合適的技術棧解決實際問題。
